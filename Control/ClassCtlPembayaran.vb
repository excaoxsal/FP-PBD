...
Public Class ClassCtlPembayaran : Implements InfProsess
    ...
    Function idBaru() As String
        ...
        Try
            DTA = New OleDbDataAdapter("Select max(right(id_pembayaran,4)) from pembayaran", BUKAKONEKSI)
            DTS = New DataSet()
            DTA.Fill(DTS, "max_id")
    ...
    Public Function cariData(kunci As String) As DataView Implements InfProsess.cariData
        Try
            DTA = New OleDbDataAdapter("Select * from PEMBAYARAN where nama_pembayaran - Like '%" & kunci & "%'", BUKAKONEKSI)
            DTS = New DataSet()
            Dim grid As New DataView(DTS.Tables("Cari_Pembayaran"))
            Return grid
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
